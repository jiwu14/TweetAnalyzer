...
def cluster_DBSCAN(sparse_dist_matrix, minPts, eps):
    ...
    return clusterer.fit_predict(sparse_dist_matrix)
...
def main():
    if len(sys.argv) == 4:
        try:
            ...
            cluster_labels = cluster_DBSCAN(tweets_sparse_dist_matrix, minPts, eps)
            ...
            for cluster_label, cluster_tweets in itertools.groupby(
                    sorted(zip(tweets, cluster_labels), key=itemgetter(1)),
                    lambda x: x[1]):
